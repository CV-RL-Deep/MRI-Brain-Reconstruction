...
class FastSurferEvaluator:
    ...
    def __init__(self, config: Config):
        ...
        self.macro_regions = {
            ...
            'Cortex': list(range(1000, 1036))
            + list(range(2000, 2036)),  # DKT Cortical regions
        }
    ...
    def convert_to_nifti(self):
        ...
        nifti_files = sorted(
            glob.glob(os.path.join(self.nifti_dir, "*.nii.gz"))
        )
        ...
        if not npy_files:
            raise FileNotFoundError(
                f"No .npy files found in {self.results_dir}. Run ablation mode first."
            )
        ...
        for f in tqdm(npy_files, desc="NIfTI Conversion"):
            vol = np.load(
                f
            )  # shape is currently (Z, Y, X) and range is [0.0, 1.0]
    ...
    def run_prediction(self):
        ...
        env["PYTHONPATH"] = (
            os.path.abspath("FastSurfer") + ":" + env.get("PYTHONPATH", "")
        )
        ...
        for f in tqdm(nifti_files, desc="FastSurfer Inference"):
            ...
            out_file = os.path.join(
                self.seg_dir, sid, "mri", "aparc.DKTatlas+aseg.deep.mgz"
            )
            ...
            cmd = [
                ...
                "--sd",
                os.path.abspath(self.seg_dir),  # subject directory root
                "--sid",
                sid,  # subject ID
                "--device",
                "cuda",  # force GPU
                "--batch",
                "16",  # massive speedup for the neural network
                "--threads",
                "4",  # speedup for CPU conforming steps
            ]
    ...
    def calculate_dsc(self):
        ...
        gt_sids = [
            ...
            for f in glob.glob(os.path.join(self.nifti_dir, "gt_*.nii.gz"))
        ]
        ablations = [
            'baseline',
            'spade',
        ...
        for gt_sid in tqdm(gt_sids, desc="Scoring Volumes"):
            ...
            gt_path = os.path.join(
                self.seg_dir, gt_sid, "mri", "aparc.DKTatlas+aseg.deep.mgz"
            )
