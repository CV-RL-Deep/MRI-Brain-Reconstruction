...
class FrechetEvaluator:
    ...
    def __init__(self, config: Config):
        ...
        self.i3d = video_models.mvit_v1_b(
            weights=video_models.MViT_V1_B_Weights.DEFAULT
        )
        self.i3d.head = (
            torch.nn.Identity()
        )  # Strip classification head to get raw features
    ...
    def _extract_features(
        ...
        for z_start in range(0, max(1, Z - chunk_size + 1), stride):
            ...
            if vol_clip.shape[0] < chunk_size:
                ...
                vol_clip = np.pad(
                    vol_clip, ((0, pad_width), (0, 0), (0, 0)), mode='edge'
                )
            ...
            vol_tensor = (
                torch.tensor(vol_clip, dtype=torch.float32)
                .unsqueeze(0)
                .unsqueeze(0)
            )
            ...
            vol_tensor = torch.nn.functional.interpolate(
                vol_tensor, size=target_size, mode='trilinear'
            )
    ...
    def evaluate(self):
        ...
        if not gt_files:
            logger.error(
                f"No Ground Truth (gt_*.npy) files found in {self.results_dir}. Aborting."
            )
            return
        ablations = [
            'baseline',
        ...
        for gt_path in tqdm(gt_files, desc="Extracting 3D Features"):
            vol_id = (
                os.path.basename(gt_path).replace('gt_', '').replace('.npy', '')
            )
            ...
            features_r3d['gt'].extend(
                self._extract_features(
                    gt_vol, self.r3d, target_size=(16, 112, 112)
                )
            )
            features_i3d['gt'].extend(
                self._extract_features(
                    gt_vol, self.i3d, target_size=(16, 224, 224)
                )
            )
            ...
            for ab in ablations:
                ...
                features_r3d[ab].extend(
                    self._extract_features(
                        pred_vol, self.r3d, target_size=(16, 112, 112)
                    )
                )
                features_i3d[ab].extend(
                    self._extract_features(
                        pred_vol, self.i3d, target_size=(16, 224, 224)
                    )
                )
        ...
        # Ground Truth Stats
        mu_gt_r3d, sig_gt_r3d = (
            np.mean(features_r3d['gt'], axis=0),
            np.cov(features_r3d['gt'], rowvar=False),
        )
        mu_gt_i3d, sig_gt_i3d = (
            np.mean(features_i3d['gt'], axis=0),
            np.cov(features_i3d['gt'], rowvar=False),
        )

        for ab in ablations:
            if not features_r3d[ab]:
                continue

            mu_ab_r3d, sig_ab_r3d = (
                np.mean(features_r3d[ab], axis=0),
                np.cov(features_r3d[ab], rowvar=False),
            )
            mu_ab_i3d, sig_ab_i3d = (
                np.mean(features_i3d[ab], axis=0),
                np.cov(features_i3d[ab], rowvar=False),
            )

            fid_3d = self._calculate_frechet_distance(
                mu_gt_r3d, sig_gt_r3d, mu_ab_r3d, sig_ab_r3d
            )
            fvd = self._calculate_frechet_distance(
                mu_gt_i3d, sig_gt_i3d, mu_ab_i3d, sig_ab_i3d
            )

            results.append({'Ablation': ab, '3D-FID': fid_3d, 'FVD': fvd})
