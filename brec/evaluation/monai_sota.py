...
from ..core.utils import logger, InferenceProfiler
...
class MonaiSotaEvaluator:
    ...
    def setup(self):
        ...
        try:
            ...
            logger.info(
                "Force-installing MONAI without touching Kaggle's CUDA/Torch deps..."
            )
            subprocess.run(
                [
                    "pip",
    ...
    def _load_3d_models(self):
        if self.autoencoder_3d is not None:
            return
        import torch
        ...
        parser = ConfigParser()
        parser.read_config(
            os.path.join(bundle_path, "configs", "inference.json")
        )
        ...
        self.scheduler_3d = DDIMScheduler(
        ...
        ae_sd = torch.load(
            os.path.join(models_dir, "autoencoder.pt"), map_location=self.device
        )

        unet_weights = [
            f
            for f in glob.glob(os.path.join(models_dir, "*.pt"))
            if "autoencoder" not in f
        ][0]
        unet_sd = torch.load(unet_weights, map_location=self.device)

        translated_ae_sd = {
            k.replace(".conv.conv.", ".postconv.conv.")
            if "decoder." in k
            else k: v
            for k, v in ae_sd.items()
        }
        translated_unet_sd = {
            k.replace(".to_out.0.", ".out_proj."): v for k, v in unet_sd.items()
        }
    ...
    def _load_2d_models(self):
        ...
        self.scheduler_2d = DDIMScheduler(
            num_train_timesteps=1000,
        ...
        self.unet_2d.load_state_dict(
            torch.load(unet_weights, map_location=self.device), strict=False
        )
        self.unet_2d.eval().to(self.device)

    def _repaint_3d_latent(
        self, vol_tensor, mask_tensor, num_inference_steps=50, repaint_jumps=2
    ):
        import torch
        ...
        latent_mask = F.interpolate(
            mask_tensor, size=latent_gt.shape[2:], mode='nearest'
        )
        latent_pred = torch.randn_like(latent_gt).to(self.device)

        dummy_cond = torch.zeros(
            (1, 4, *latent_gt.shape[2:]),
            dtype=latent_gt.dtype,
            device=self.device,
        )
        dummy_context = torch.zeros(
            (1, 1, 4), dtype=latent_gt.dtype, device=self.device
        )
        ...
        with torch.cuda.amp.autocast():
            for t in self.scheduler_3d.timesteps:
                for jump in range(repaint_jumps):
                    ...
                    noise_pred = (
                        step_out[0] if isinstance(step_out, tuple) else step_out
                    )
                    step_res = self.scheduler_3d.step(
                        noise_pred, t, latent_pred
                    )
                    latent_pred = (
                        step_res[0] if isinstance(step_res, tuple) else step_res
                    )

                    noise = torch.randn_like(latent_gt).to(self.device)
                    known_latent_t = self.scheduler_3d.add_noise(
                        latent_gt,
                        noise,
                        torch.tensor([t], dtype=torch.long, device=self.device),
                    )
                    latent_pred = (
                        known_latent_t * (1.0 - latent_mask)
                        + latent_pred * latent_mask
                    )

        with torch.no_grad():
            return self.autoencoder_3d.decode_stage_2_outputs(latent_pred)

    def _repaint_2d_pixel(
        self, slice_tensor, mask_tensor, num_inference_steps=50
    ):
        import torch
        ...
        with torch.cuda.amp.autocast():
            for t in self.scheduler_2d.timesteps:
                ...
                img_pred = (
                    known_img_t * (1.0 - mask_tensor) + img_pred * mask_tensor
                )

        return img_pred

    def evaluate_sota_models(
        self, ixi_files: list, brats_pool: list, num_volumes: int = 10
    ):
        import torch
        import torch.nn.functional as F
        ...
        for f in test_files:
            vol_id = (
                os.path.basename(f).replace('.nii.gz', '').replace('.nii', '')
            )
            ...
            eval_window_mask = mask_np[
                center - rollout_span : center + rollout_span
            ]

            if np.sum(eval_window_mask) < 10:
                logger.warning(
                    f"Skipping {vol_id}: Tumor injection failed or missed evaluation window."
                )
                continue

            # Save GT and Mask so FrechetEvaluator and plots can use them
            np.save(
                os.path.join(self.results_dir, f"gt_{vol_id}.npy"), vol_obj.t1
            )
            np.save(
                os.path.join(self.results_dir, f"mask_{vol_id}.npy"), mask_np
            )
        ...
        if not eval_data:
            logger.error(
                "No valid volumes could be prepared for SOTA evaluation!"
            )
            return
        ...
        weights_path = os.path.join(
            self.cfg.data.weights_dir, "model_full.keras"
        )
        if not os.path.exists(weights_path):
            weights_path = os.path.join(
                self.cfg.data.results_dir, "model_full.keras"
            )
        ...
        for vol_id, vol_obj, mask_np in tqdm(eval_data, desc="Ours Masked"):
            out_path = os.path.join(
                self.results_dir, f"ours_full_masked_{vol_id}.npy"
            )
        ...
        #     mask_np = VisualizationSuite._sample_real_brats_mask(vol_obj.t1, brats_pool)
        #     if np.sum(mask_np) < 10: continue
        for vol_id, vol_obj, mask_np in tqdm(
            eval_data, desc="MONAI 3D Inference"
        ):
            out_ldm = os.path.join(
                self.results_dir, f"monai_3d_ldm_{vol_id}.npy"
            )
            out_vq = os.path.join(self.results_dir, f"monai_vqgan_{vol_id}.npy")

            if os.path.exists(out_ldm) and os.path.exists(out_vq):
                continue

            vol_np = (vol_obj.t1 * 2.0) - 1.0
            vol_t = (
                torch.tensor(vol_np, dtype=torch.float32)
                .unsqueeze(0)
                .unsqueeze(0)
                .to(self.device)
            )
            ...
            pad_z, pad_y, pad_x = (
                max(0, 128 - vol_t.shape[2]),
                max(0, 160 - vol_t.shape[3]),
                max(0, 160 - vol_t.shape[4]),
            )
            vol_t_pad = F.pad(
                vol_t,
            ...
            if not os.path.exists(out_vq):
                prof_vqgan.start()
                with torch.no_grad():
                    latent = self.autoencoder_3d.encode_stage_2_inputs(
                        vol_t_pad
                    )
                    pred_vq_pad = self.autoencoder_3d.decode_stage_2_outputs(
                        latent
                    )
                prof_vqgan.stop_and_log(vol_id)

                pred_vq = pred_vq_pad[
                    :, :, : vol_t.shape[2], : vol_t.shape[3], : vol_t.shape[4]
                ]
                pred_vq_np = np.clip(
                    (pred_vq.squeeze().cpu().numpy() + 1.0) / 2.0, 0.0, 1.0
                )
                np.save(
                    out_vq,
                    (pred_vq_np * mask_np) + (vol_obj.t1 * (1.0 - mask_np)),
                )

            # 3D LDM Inference
            if not os.path.exists(out_ldm):
                prof_3d_ldm.start()
                pred_ldm_pad = self._repaint_3d_latent(
                    vol_t_pad, mask_t_pad, num_inference_steps=50
                )
                prof_3d_ldm.stop_and_log(vol_id)

                pred_ldm = pred_ldm_pad[
                    :, :, : vol_t.shape[2], : vol_t.shape[3], : vol_t.shape[4]
                ]
                pred_ldm_np = np.clip(
                    (pred_ldm.squeeze().cpu().numpy() + 1.0) / 2.0, 0.0, 1.0
                )
                np.save(
                    out_ldm,
                    (pred_ldm_np * mask_np) + (vol_obj.t1 * (1.0 - mask_np)),
                )
        ...
        #     if np.sum(mask_np) < 10: continue
        for vol_id, vol_obj, mask_np in tqdm(
            eval_data, desc="MONAI 2D Inference"
        ):
            out_2d = os.path.join(
                self.results_dir, f"monai_2d_ldm_{vol_id}.npy"
            )
            ...
            active_z = [
                z for z in range(vol_np.shape[0]) if np.sum(mask_np[z]) > 0
            ]
            ...
            if active_z:
                ...
                for b_start in range(0, len(active_z), mini_batch_size):
                    ...
                    slices_t = (
                        torch.tensor(vol_np[chunk_z], dtype=torch.float32)
                        .unsqueeze(1)
                        .to(self.device)
                    )
                    masks_t = (
                        torch.tensor(mask_np[chunk_z], dtype=torch.float32)
                        .unsqueeze(1)
                        .to(self.device)
                    )
                    ...
                    pad_y, pad_x = (
                        max(0, 240 - slices_t.shape[2]),
                        max(0, 240 - slices_t.shape[3]),
                    )
                    slices_pad = F.pad(
                        slices_t,
                        (0, pad_x, 0, pad_y),
                    ...
                    preds_pad = self._repaint_2d_pixel(
                        slices_pad, masks_pad, num_inference_steps=50
                    )
                    ...
                    for i, z in enumerate(chunk_z):
                        pred_vol_np[z] = (
                            preds_pad[
                                i, 0, : slices_t.shape[2], : slices_t.shape[3]
                            ]
                            .cpu()
                            .numpy()
                        )
            ...
            np.save(
                out_2d,
                (pred_final_np * mask_np) + (vol_obj.t1 * (1.0 - mask_np)),
            )
