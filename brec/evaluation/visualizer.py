...
def analyze_dataset_geometry(data_list, num_samples=3):
    ...
    for path in samples:
        ...
        fig.suptitle(
            f"{filename} (Canonical -> Transposed 2,1,0)", fontsize=10, y=0.98
        )
        ...
        # Result is Z-X plane
        axes[1].imshow(
            std_data[:, c_y, :], cmap='bone', aspect='equal'
        )  # no rotation needed if layout is Z-Y-X
...
class VolumeDashboard:
    ...
    def update(
        ...
        if slice_idx < 0 or slice_idx >= self.axis_size:
            return
        ...
        cell_draw.rectangle(
            [0, 0, self.cell_sz - 1, self.cell_sz - 1],
            outline=bevel_color,
            width=max(1, 2 // self.ds),
        )
        ...
        if tumor_mask is not None and np.any(tumor_mask):
            ...
            if is_target:
                ...
                if info['glioma_applied']:
                    draw_contour = True
                    c_color = (
                        (0, 0, 255) if info['mode'] == 'clean' else (255, 0, 0)
                    )
            ...
            if draw_contour:
                ...
                overlay = np.zeros(
                    (self.cell_sz, self.cell_sz, 4), dtype=np.uint8
                )
                overlay[boundary] = [*c_color, 255]
                cell_img.paste(
                    Image.fromarray(overlay), (0, 0), Image.fromarray(overlay)
                )
        ...
        # Right align text
        try:
            tw = cell_draw.textlength(role_str)
        except AttributeError:
            tw = cell_draw.textsize(role_str)[0]  # legacy compat
        cell_draw.text((self.cell_sz - tw - 2, 2), role_str, fill=role_color)

        # Bottom-Right: Z-Index (Sequence ID)
        z_str = f"z={z_index}"
        try:
            tw = cell_draw.textlength(z_str)
        except AttributeError:
            tw = cell_draw.textsize(z_str)[0]
        cell_draw.text(
            (self.cell_sz - tw - 2, self.cell_sz - 12), z_str, fill=(0, 255, 0)
        )
    ...
    def render(self, input_only=False):
        ...
        for i in range(self.axis_size):
            ...
            if i not in self.active_indices:
                ...
                # row = i // self.cols
                draw.text(
                    (col * self.cell_sz + 2, row * self.cell_sz + 2),
                ...
                try:
                    tw = draw.textlength(count_str)
                except AttributeError:
                    tw = draw.textsize(count_str)[0]
        ...
        draw.rectangle(
            [lx0, ly0, lx0 + self.cell_sz - 1, ly0 + self.cell_sz - 1],
            outline=(100, 100, 100),
        )
        draw.text(
            (lx0 + 4, ly0 + 2), "Global Sampling\nHeatmap", fill=(255, 255, 255)
        )
        ...
        save_path = os.path.join(
            self.dataset_dir, axis_names[self.axis], f"{base_name}.jpg"
        )
...
class VisualizationSuite:
    ...
    @staticmethod
    def _sample_real_brats_mask(
        target_volume: np.ndarray, brats_pool: list, target_z: int = None
    ) -> np.ndarray:
        """
        ...
        if not valid_brats:
            ...
            logger.warning(
                "⚠️ No BraTS volumes with masks found. Falling back to synthetic mask to prevent Teacher Forcing zero-error curves."
            )
            return VisualizationSuite._generate_synthetic_mask(
                target_volume, max_radius=18
            )
            return mask
        ...
        for _ in range(50):
            ...
            cz, cy, cx = hemisphere_coords[
                random.randint(0, len(hemisphere_coords) - 1)
            ]
            ...
            if (
                ...
                and sy >= 0
                and sx >= 0
                and ez < target_volume.shape[0]
                and ey < target_volume.shape[1]
                and ex < target_volume.shape[2]
            ):
    ...
    @staticmethod
    def _generate_synthetic_mask(
        volume: np.ndarray, max_radius=20
    ) -> np.ndarray:
    ...
    def plot_augmentations(dataset, num_groups=2, batch_mode=False):
        ...
        criteria = {
            "glioma_only": lambda s: (
                s["glioma_applied"]
                and not s["blur_applied"]
                and not s["noise_applied"]
            ),
            "blur_only": lambda s: (
                not s["glioma_applied"]
                and s["blur_applied"]
                and not s["noise_applied"]
            ),
            "noise_only": lambda s: (
        ...
        for x_batch, y_batch, info_batch in tqdm(
            dataset.take(1000), total=1000, disable=batch_mode
        ):
            if stop:
                break
        ...
        for k in range(num_groups):
            ...
            fig, axes = plt.subplots(
                num_rows, num_inputs + 1, figsize=(15, 3 * num_rows)
            )
            if num_rows == 1:
                axes = [axes]
            ...
            for i, (key, sample_list) in enumerate(samples.items()):
                if k >= len(sample_list):
                    ...
                    ax.text(
                        0.5, 0.5, f"Missing: {key}", ha='center', color='red'
                    )
                ...
                ax_first.set_ylabel(
                    key.replace('_', ' ').title(), fontsize=12, weight='bold'
                )
    ...
    def plot_training_history(history):
        ...
        for i, metric in enumerate(base_metrics):
            ...
            if val_key in metrics:
                ax.plot(
                    epochs, metrics[val_key], 'r--s', label=f'Valid {metric}'
                )
    ...
    def plot_hallucination_buffer(manager, num_samples=3):
        ...
        for i, key in enumerate(keys):
            ...
            axes[i][0].imshow(gt, cmap='bone')
            axes[i][0].set_title("Ground Truth")
            axes[i][1].imshow(pred, cmap='bone')
            axes[i][1].set_title("Prediction")
            axes[i][2].imshow(np.abs(gt - pred), cmap='inferno')
    ...
    def analyze_best_worst(model, val_dataset, num_samples=3):
        ...
        for x_batch, y_batch in iterator:
            ...
            inputs = (
                x_batch['history_input'].numpy()
                if hasattr(x_batch['history_input'], 'numpy')
                else x_batch['history_input']
            )
            targets = y_batch.numpy() if hasattr(y_batch, 'numpy') else y_batch

            # Calculate MSE per sample in batch
            batch_mse = np.mean(
                np.square(targets[..., 0] - preds[..., 0]), axis=(1, 2)
            )
            ...
            for i in range(targets.shape[0]):
                ...
                if len(best_samples) > num_samples:
                    best_samples.pop()
        ...
        def visualize_group(samples, title):
            ...
            fig, axes = plt.subplots(
                n_vis, cols, figsize=(3 * cols, 3.75 * n_vis)
            )
        ...
        if best_samples:
            visualize_group(best_samples, "Best Predictions (Lowest Error)")
        if worst_samples:
            visualize_group(worst_samples, "Worst Predictions (Highest Error)")

    @staticmethod
    def plot_autoregressive_performance(
        reconstructor, file_info, manager, masked_inference=False
    ):
        ...
        recon_fwd = reconstructor.autoregressive_restore(
            vol, center, fwd_end, 'forward', mask_volume=mask_vol
        )
        if mask_vol is not None:
        ...
        recon_bwd = reconstructor.autoregressive_restore(
            vol, bwd_start, center, 'backward', mask_volume=mask_vol
        )

        # 4. Error Curves
        fwd_mse = np.mean(
            np.square(vol[center:fwd_end] - recon_fwd[center:fwd_end]),
            axis=(1, 2),
        )
        bwd_mse = np.mean(
            np.square(vol[bwd_start:center] - recon_bwd[bwd_start:center]),
            axis=(1, 2),
        )

        plt.figure(figsize=(14, 6))
        plt.plot(
            np.arange(bwd_start, center), bwd_mse, 'g-o', label='Backward (<-)'
        )
        plt.plot(
            np.arange(center, fwd_end), fwd_mse, 'r-o', label='Forward (->)'
        )
        plt.axvline(
            x=center, color='b', linestyle='--', label='Start (Center Slice)'
        )
        ...
        if mask_vol is not None:
            ...
            plt.title(
                f"Bidirectional Autoregressive Error Accumulation: {filename}"
            )
        ...
        fig, axes = plt.subplots(
            rows, cols, figsize=(2.5 * cols, 4.2 * rows)
        )  # increase height
        if mask_vol is not None:
            fig.suptitle(
                f"Reconstruction Details (Masked): {filename}",
                y=0.96,
                fontsize=16,
            )
        else:
            fig.suptitle(
                f"Reconstruction Details: {filename}", y=0.96, fontsize=16
            )
        ...
        for i, idx in enumerate(idx_show):
            ...
            ax.imshow(
                np.abs(vol[idx] - img_r), cmap='inferno', vmin=0, vmax=0.3
            )
    ...
    def plot_bidirectional(
        ...
        if masked_inference and seg is None:
            print(
                "WARNING: No real mask found. Generating synthetic spherical mask for IXI evaluation."
            )
        ...
        print(
            f"Tumor Range found: Slice {start} to {end} (Total {end - start} slices)"
        )
        ...
        recon_fwd = reconstructor.autoregressive_restore(
            t1, start, end, 'forward', mask_volume=mask
        )

        # 4. Run Backward Pass (Top -> Bottom)
        # Context comes from [end ... end+N]
        recon_bwd = reconstructor.autoregressive_restore(
            t1, start, end, 'backward', mask_volume=mask
        )
        ...
        if end - start < num_vis_slices:
            ...
            vis_indices = np.linspace(
                start, end - 1, num=num_vis_slices, dtype=int
            )
        ...
        if not masked_inference:
            fig.suptitle(
                f"Bidirectional Autoregressive Inpainting: {brats_file['id']}",
                fontsize=16,
                y=0.98,
            )
        else:
            fig.suptitle(
                f"Bidirectional (Masked) Inpainting: {brats_file['id']}",
                fontsize=16,
                y=0.98,
            )
        ...
        for i, idx in enumerate(vis_indices):
            ...
            if np.sum(seg_slice) > 0:
                ...
                row_axes[0].contour(
                    seg_slice, levels=[0.5], colors='red', linewidths=0.5
                )
    ...
    def plot_sampling_statistics(manager):
        ...
        top_vols = random.sample(
            tuple(vol_counts.keys()),
            counts=tuple(vol_counts.values()),
            k=k_samples,
        )
        ...
        slice_counts = {
            'ixi': {0: Counter(), 1: Counter(), 2: Counter()},
        ...
        for dataset in ['ixi', 'brats']:
            ...
            fig.suptitle(
                f"{dataset.upper()} Slice Distribution (Aggregated)",
                fontsize=16,
            )
...
def display_random_dashboards(base_dirs=None):
    ...
    if base_dirs is None:
        base_dirs = [
            d
            for d in os.listdir('.')
            if os.path.isdir(d) and d.startswith('eda_')
        ]
