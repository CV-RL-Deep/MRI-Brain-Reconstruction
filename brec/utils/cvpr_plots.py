...
def generate_figure_1_drift_curve():
    ...
    is_interactive = (
        os.environ.get('KAGGLE_KERNEL_RUN_TYPE', 'Interactive') == 'Interactive'
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(figures_dir, 'Fig1_DriftCurve_SSIM.pdf'),
        dpi=300,
        bbox_inches='tight',
    )
    plt.close(fig_ssim)
    ...
    if 'FFL' in df.columns:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(figures_dir, 'Fig1_DriftCurve_FFL.pdf'),
            dpi=300,
            bbox_inches='tight',
        )
        plt.close(fig_ffl)
        ...
        print(
            "  -> Skipping FFL plot (FFL metric not found in CSV. Re-run ablation inference)."
        )
...
def generate_figure_2_evolution_matrix():
    ...
    for vol_idx, npy_file in enumerate(
        npy_files[: 5 if is_interactive else None]
    ):
        vol_id = (
            os.path.basename(npy_file).replace('full_', '').replace('.npy', '')
        )
        ...
        if os.path.exists(gt_path):
            ...
            search_paths = glob.glob(
                f"/kaggle/input/**/{vol_id}", recursive=True
            ) + glob.glob(f"data/**/{vol_id}", recursive=True)
            if search_paths:
                gt_obj = VolumeLoader.load(search_paths[0])
                if gt_obj:
                    gt_vol = gt_obj.t1
        ...
        is_interactive = (
            os.environ.get('KAGGLE_KERNEL_RUN_TYPE', 'Interactive')
            == 'Interactive'
        )
        ...
        for r, ab in enumerate(row_keys):
            for c, k in enumerate(k_steps):
                ...
                if ab in vols:
                    ...
                    ax.imshow(
                        vols[ab][start_idx + k], cmap='bone', vmin=0, vmax=1
                    )
                ax.axis('off')
        ...
        plt.savefig(
            os.path.join(
                figures_dir, f"Fig2_EvolutionMatrix_vol{vol_idx + 1:03d}.pdf"
            ),
            dpi=300,
            bbox_inches='tight',
            pad_inches=0.01,
...
def generate_figure_3_pe_proof():
    ...
    for vol_idx, npy_file in enumerate(
        ...
        try:
            vol_buffer = np.load(
                os.path.join(results_dir, f'buffer_{vol_id}.npy')
            )
        ...
        fig, axes = plt.subplots(
            ...
            figsize=(
                3.25,
                3.25 * (1.0 * gt_slice.shape[0] / gt_slice.shape[1]),
            ),
        ...
        plt.savefig(
            os.path.join(
                figures_dir, f"Fig3_PE_Proof_vol{vol_idx + 1:03d}.pdf"
            ),
            dpi=300,
            bbox_inches='tight',
            pad_inches=0.01,
...
def generate_figure_4_anatomical_dsc():
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(figures_dir, 'Fig4_Anatomical_DSC.pdf'),
        dpi=300,
        bbox_inches='tight',
    )
    plt.close(fig)
...
def generate_figure_5_frechet_distances():
    ...
    if not os.path.exists(csv_path):
        print(
            f"Error: Could not find {csv_path}. Did you run the 'frechet' mode?"
        )
        return
    ...
    # Double column width = 6.875 inches
    fig, axes = plt.subplots(
        1, 2, figsize=(6.875, 2.5), gridspec_kw={'wspace': 0.35}
    )

    sns.barplot(data=df, x='Ablation', y='3D-FID', palette=PALETTE, ax=axes[0])
    axes[0].set_xlabel('')
    ...
    axes[0].set_xticklabels(
        [
            LABELS.get(l.get_text(), l.get_text())
    ...
    # Plot 2: FVD
    sns.barplot(data=df, x='Ablation', y='FVD', palette=PALETTE, ax=axes[1])
    axes[1].set_xlabel('')
    ...
    plt.savefig(
        os.path.join(figures_dir, 'Fig5_Frechet_Distances.pdf'),
        dpi=300,
        bbox_inches='tight',
    )
    plt.close(fig)
...
def generate_figure_6_pareto(combined=True):
    ...
    df_master = (
        df_perf.groupby('Model')
        .agg({'Time_Seconds': 'mean', 'Peak_VRAM_GB': 'mean'})
        .reset_index()
    )
    ...
    if os.path.exists(fid_csv):
        ...
        df_master = pd.merge(
            df_master, df_fid, left_on='Model', right_on='Ablation', how='left'
    ...
    if os.path.exists(dsc_csv):
        ...
        df_master = pd.merge(
            df_master, df_dsc, left_on='Model', right_on='Ablation', how='left'
        ).drop(columns=['Ablation'])
    ...
    if os.path.exists(abl_csv):
        df_abl = (
            pd.read_csv(abl_csv)
            .groupby('Ablation')
            .agg({'SSIM': 'mean', 'MAE': 'mean'})
            .reset_index()
        )
        df_master = pd.merge(
            df_master, df_abl, left_on='Model', right_on='Ablation', how='left'
        ).drop(columns=['Ablation'])
    ...
    if df_merged.empty:
        print(
            "ERROR: df_merged is empty! Check if the Model names in inference_performance.csv match Ablation names."
        )
        return
    ...
    available_metrics = [
        (col, lbl) for col, lbl in metrics_to_plot if col in df_merged.columns
    ]
    ...
    if combined and available_metrics:
        ...
        for i, (metric_col, metric_label) in enumerate(available_metrics):
            ...
            axes[i, 0].set_xlabel(
                r'Inference Time per Volume (Seconds) $\downarrow$'
            )
            axes[i, 0].set_ylabel(metric_label)
            if i == 0:
                axes[i, 0].set_title('Compute Time vs. Fidelity', fontsize=9)
            ...
            if i == 0:
                axes[i, 1].set_title(
                    'Memory Footprint vs. Fidelity', fontsize=9
                )
        ...
        plt.savefig(
            os.path.join(figures_dir, 'Fig6_Pareto_Combined.pdf'),
            dpi=300,
            bbox_inches='tight',
        )
        plt.close(fig)
        ...
        for metric_col, metric_label in available_metrics:
            ...
            fig, axes = plt.subplots(
                1, 2, figsize=(6.875, 2.75), gridspec_kw={'wspace': 0.35}
            )
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(figures_dir, f'Fig6_Pareto_{metric_col}.pdf'),
                dpi=300,
                bbox_inches='tight',
            )
            plt.close(fig)
...
def generate_supp_bidirectional():
    ...
    if not weight_files:
        weight_files = glob.glob(
            os.path.join(results_dir, '..', '*-???e.keras')
        )

    if not weight_files:
        print(
            "  -> No epoch specific weights found. Skipping Supp Bidirectional."
        )
        return
    ...
    if not vol_id:
        vol_id = (
            os.path.basename(test_brats['t1'])
            .replace('_t1.nii.gz', '')
            .replace('.nii', '')
        )
    ...
    for weight_path in sorted(weight_files):
        ...
        fig, axes = plt.subplots(
            num_vis_slices,
            5,
            figsize=(6.875, 1.2 * num_vis_slices),
        ...
        for i, idx in enumerate(vis_indices):
            ...
            axes[i, 3].imshow(
                np.abs(gt_slice - recon_fwd[idx]),
                cmap='inferno',
                vmin=0,
                vmax=0.3,
            )
            axes[i, 4].imshow(
                np.abs(gt_slice - recon_bwd[idx]),
                cmap='inferno',
                vmin=0,
                vmax=0.3,
            )
        ...
        plt.savefig(
            os.path.join(figures_dir, f'Supp_Bidirectional_{epoch_id}.pdf'),
            dpi=150,
            bbox_inches='tight',
        )
        plt.close()
...
def generate_supp_ablation_masked_ar():
    ...
    synthetic_mask = VisualizationSuite._sample_real_brats_mask(
        vol, brats_pool, target_z=center
    )
    ...
    for ab_name, cfg in configs.items():
        ...
        fwd_mse = np.mean(
            np.square(vol[start:end] - recon_fwd[start:end]), axis=(1, 2)
        )
        bwd_mse = np.mean(
            np.square(vol[start:end] - recon_bwd[start:end]), axis=(1, 2)
        )
        ...
        plt.savefig(
            os.path.join(figures_dir, f'Supp_Masked_Curve_{ab_name}.pdf'),
            dpi=150,
            bbox_inches='tight',
        )
        plt.close()
        ...
        fig.suptitle(
            f"Reconstruction Details [{safe_ab}]",
            fontsize=11,
            weight='bold',
            y=0.98,
        )
        ...
        for i, idx in enumerate(idx_show):
            ...
            for j in range(N):
                ...
                if 0 <= c_idx < vol.shape[0]:
                    ...
                    if i == 0:
                        ax.set_title(f"Ctx {j + 1}", fontsize=8)
                ax.axis('off')
            ...
            if i == 0:
                axes[i, N].set_title("Mask", fontsize=8)
            axes[i, N].axis('off')

            # GT
            axes[i, N + 1].imshow(vol[idx], cmap='bone', vmin=0, vmax=1)
            if i == 0:
                axes[i, N + 1].set_title("GT", fontsize=8)
            axes[i, N + 1].axis('off')

            # Pred
            axes[i, N + 2].imshow(img_r, cmap='bone', vmin=0, vmax=1)
            if i == 0:
                axes[i, N + 2].set_title("Pred", fontsize=8)
            axes[i, N + 2].axis('off')

            # Diff
            mse = np.mean((vol[idx] - img_r) ** 2)
            axes[i, N + 3].imshow(
                np.abs(vol[idx] - img_r), cmap='inferno', vmin=0, vmax=0.2
            )
            if i == 0:
                axes[i, N + 3].set_title("Error", fontsize=8)
            axes[i, N + 3].axis('off')
        ...
        plt.savefig(
            os.path.join(figures_dir, f'Supp_Masked_Details_{ab_name}.pdf'),
            dpi=150,
            bbox_inches='tight',
        )
        plt.close()
...
def generate_supp_scaling_failure(render_errors=True):
    ...
    b0_weights = os.path.abspath(
        os.path.join(weights_dir, '..', 'ablations-B0', 'model_full.keras')
    )
    b1_weights = os.path.abspath(
        os.path.join(weights_dir, '..', 'ablations-B1', 'model_full.keras')
    )

    if not os.path.exists(b0_weights) or not os.path.exists(b1_weights):
        print(
            f"  -> B0 or B1 weights not found. Skipping scaling failure plot.\n"
    ...
    for vol_idx, test_ixi in enumerate(ixi_files[:num_eval_vols]):
        ...
        with tf.device('/CPU:0'):
            pred_b0 = recon_b0.autoregressive_restore(
                vol, start, end, 'forward'
            )
            pred_b1 = recon_b1.autoregressive_restore(
                vol, start, end, 'forward'
            )
        ...
        vis_indices = np.linspace(
            start + 2, end - 2, num=num_vis_slices, dtype=int
        )
        ...
        for i, idx in enumerate(vis_indices):
            ...
            if render_errors:
                axes[i, 3].imshow(
                    np.abs(gt_slice - p_b0), cmap='inferno', vmin=0, vmax=0.3
                )
                axes[i, 4].imshow(
                    np.abs(gt_slice - p_b1), cmap='inferno', vmin=0, vmax=0.3
                )
        ...
        plt.savefig(
            os.path.join(
                figures_dir, f'Supp_Scaling_Failure_vol{vol_idx + 1:03d}.pdf'
            ),
            dpi=150,
            bbox_inches='tight',
        )
...
def run_cvpr_rendering(render_supp=True, render_bidirectional=False):
    ...
    if render_supp:
        ...
        print(
            "Skipping Supplementary Figures to save time (render_supp=False)."
        )
