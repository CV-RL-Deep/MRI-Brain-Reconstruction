...
from ..models.losses import CompositeLoss, get_perceptual_loss
...
def compute_autoregressive_score(
    model, config, val_loader, ixi_files, brats_files, n_steps=5
):
    """
    ...
    for d in brats_files:
        vol = next(
            (v for v in val_loader.pool['brats'] if v.path == d['t1']), None
        )
        if vol:
            test_volumes.append(vol)
    ...
    for vol_obj in test_volumes:
        ...
        recon = reconstructor.autoregressive_restore(
...
class HPMEngine:
    ...
    def __init__(self, config: Config, train_files: list, brats_files: list):
        ...
        self.val_brats = brats_files[
            n_train_brats : n_train_brats + n_valid_brats
        ]
    ...
    def _objective(self, trial):
        ...
        if not self.cfg.train.gan_mode:
            ...
            if not self.cfg.train.use_spatial_loss:
                ...
                self.loss_fn.w_tumor.assign(
                    trial.suggest_float('l_tumor', 0.0, 1.0)
                )
                self.loss_fn.w_healthy.assign(
                    trial.suggest_float('l_healthy', 0.0, 1.0)
                )
                self.loss_fn.w_bg.assign(
                    trial.suggest_float('l_background', 0.0, 1.0)
                )
                self.loss_fn.w_grad.assign(
                    trial.suggest_float('l_grad', 0.0, 1.0)
                )
                self.loss_fn.w_perc.assign(
                    trial.suggest_float('l_perceptual', 0.0, 1.0)
                )
                self.loss_fn.w_spec.assign(
                    trial.suggest_float('l_spectral', 0.0, 1.0)
                )
        else:
            ...
            self.trainer.model.w_l1.assign(
                trial.suggest_float('w_l1', 0.0, 1.0)
            )
            self.trainer.model.w_perc.assign(
                trial.suggest_float('w_perc', 0.0, 1.0)
            )
            self.trainer.model.w_spec.assign(
                trial.suggest_float('w_spec', 0.0, 1.0)
            )
            self.trainer.model.w_gan.assign(
                trial.suggest_float('w_gan', 0.0, 1.0)
            )
            self.trainer.model.w_fm.assign(
                trial.suggest_float('w_fm', 0.0, 1.0)
            )

            if self.cfg.model.architecture == 'vae':
                self.trainer.model.w_kl.assign(
                    trial.suggest_float('w_kl', 0.0, 0.1)
                )
        ...
        try:
            ...
            if history and 'val_loss' in history.history:
                ...
                history and 'val_l1_loss' in history.history
            ):  # fallback for GAN mode
            ...
            if np.isnan(history.history['loss'][-1]):
                ...
                ar_score = compute_autoregressive_score(
                    ...
                    self.eval_ixi,
                    self.eval_brats,
                    n_steps=self.cfg.hpo.ar_rollout_steps,
                )
    ...
    def run(self):
        ...
        if os.path.isfile(self.cfg.hpo.database_preload) and not os.path.exists(
            final_db_name
        ):
            logger.info(
                f"Found preload database: {self.cfg.hpo.database_preload}"
            )
        ...
        if os.path.exists(final_db_name):
            ...
            logger.info(
                f"Loaded previous Optuna history from sqlite:///{final_db_name}"
            )
        else:
        ...
        logger.info(
            f"Allocating {len(self.train_ixi)} IXI and {len(self.train_brats)} BraTS volumes to RAM..."
        )

        self.val_loader = StaticLoader(self.cfg, self.val_ixi, self.val_brats)
        self.train_loader = StaticLoader(
            self.cfg, self.train_ixi, self.train_brats
        )
        ...
        self.train_gen_brats = BraTSActiveGenerator(
            self.train_loader, mode='clean'
        )
        ...
        val_gen = SequentialValidationGenerator(
            self.val_loader, max_slices_per_vol=10
        )
        ...
        if (
            self.cfg.train.lambda_perceptual >= 0.01
            or self.cfg.train.weight_perceptual >= 0.01
        ):
            logger.info("Pre-warming Global Perceptual Loss Singleton...")
            get_perceptual_loss(self.cfg)
        else:
            logger.info(
                "Perceptual Loss weight < 0.01. Bypassing EfficientNet initialization."
            )
        ...
        self.model.compile(
            optimizer=optimizer, loss=self.loss_fn, metrics=metrics
        )
        ...
        study.optimize(self._objective, n_trials=self.cfg.hpo.n_trials)
    ...
    def _save_best_config(self, best_params):
        ...
        if 'l_tumor' in best_params:
            self.cfg.train.lambda_tumor = best_params['l_tumor']
        if 'l_healthy' in best_params:
            self.cfg.train.lambda_healthy = best_params['l_healthy']
        if 'l_background' in best_params:
            self.cfg.train.lambda_background = best_params['l_background']
        if 'l_grad' in best_params:
            self.cfg.train.lambda_grad = best_params['l_grad']
        if 'l_perceptual' in best_params:
            self.cfg.train.lambda_perceptual = best_params['l_perceptual']
        if 'l_spectral' in best_params:
            self.cfg.train.lambda_spectral = best_params['l_spectral']
        if 'lr_g' in best_params:
            self.cfg.train.learning_rate_g = best_params['lr_g']
        if 'lr_d' in best_params:
            self.cfg.train.learning_rate_d = best_params['lr_d']
        if 'w_l1' in best_params:
            self.cfg.train.weight_l1 = best_params['w_l1']
        if 'w_perc' in best_params:
            self.cfg.train.weight_perceptual = best_params['w_perc']
        if 'w_spec' in best_params:
            self.cfg.train.weight_spectral = best_params['w_spec']
        if 'w_gan' in best_params:
            self.cfg.train.weight_gan = best_params['w_gan']
